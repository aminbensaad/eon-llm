...
def main(model_name, input_path, output_path):
    ...
    # Disk space check could be added if needed

    logger.info("Loading the model and tokenizer...")
    # Load the model and tokenizer with local_files_only set to True first
    try:
        ...
        logger.info(
            "Model or tokenizer not found locally. Downloading from Hugging Face..."
        )
        logger.error(e)
    ...
    def generate_answer(question, context):
        ...
        for input_ in inputs:
            try:
                prompt = f"<s>[INST] Kontext: {input_['context']} Frage: {input_['question']} [/INST]"
                output = qa_pipeline(
                    prompt,
                    max_length=2048,
                    do_sample=True,
