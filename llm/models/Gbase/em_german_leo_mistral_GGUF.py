...
def main(model_name, input_path, output_path):
    ...
    if torch.cuda.is_available():
        n_gpu_layers = 35  # You can adjust this value based on your GPU capability
    ...
    model_file_name = "em_german_leo_mistral.Q4_K_M.gguf"
    ...
    def generate_answer(question, context):
        prompt = f"Du bist ein hilfreicher Assistent. USER: {question} ASSISTANT:"
        output = llm(
            prompt,  # Prompt
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print(
            "Usage: python em_german_leo_mistral-GGUF.py <model_name> <input_path> <output_path>"
        )
        sys.exit(1)
